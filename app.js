...
function rebuildMeetingOptions() {
  ...
  // Sort by date first, then firstRaceTime for same day
  filteredMeetings.sort((a, b) => {
    const dateA = new Date(`${a.date}T${a.firstRaceTime || '00:00'}`);
    const dateB = new Date(`${b.date}T${b.firstRaceTime || '00:00'}`);
    return dateA - dateB;
  });
